...
static int showall = 0, showframe = 0, frame = 0;
...
#else
            sf.pixels[ix+iy*sf.w] = (int)(sin(z/5)*127+127)*0x10101;
#endif
        }
    }
}

int normals_main(int argc, char**argv) {
    if (argc > 1) {
    ...
    pstart = new int[N_FRAMES+1];
    pose = new double[N_FRAMES*3];
    ...
    for (int i = 0; i < N_FRAMES; i++) {
        ...
        pose[i*3]   = dx*1000.f;
        pose[i*3+1] = dy*1000.f;
        pose[i*3+2] = dz*1000.f;
    ...
    while (1) {
        while (screen.gotEvent()) {
            ...
            if (e.type == KeyPress and e.key == K_ESCAPE) return 0;
                ...
                else if (e.key == K_p) cout << frame << endl;
            }
            track.handleEvent(e);
        }
        track.update();
